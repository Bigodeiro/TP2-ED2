...
void Particao(FILE **ArqLi, FILE **ArqEi, FILE **ArqLEs, TipoArea Area, int Esq, int Dir, int *i, int *j, Dados * Dados){
    ...
    *j = Dir + 1;
    ...
    while(Ls >= Li){
        if(NRArea < TAMAREA - 1){
            if(OndeLer) LeSup(ArqLEs, &UltLido, &Ls, &OndeLer, Dados);
            else LeInf(ArqLi, &UltLido, &Li, &OndeLer, Dados);
        ...
        if(Ls == Es) LeSup(ArqLEs, &UltLido, &Ls, &OndeLer, Dados);
        else if (Li == Ei) LeInf(ArqLi, &UltLido, &Li, &OndeLer, Dados);
        else if (OndeLer) LeSup(ArqLEs, &UltLido, &Ls, &OndeLer, Dados);
        else LeInf(ArqLi, &UltLido, &Li, &OndeLer, Dados);
        ...
        Dados->acessos.comparacoesChave += 1;
